 ...
 *
 * @see delphinus_content_width() for template-specific adjustments.
 */
if ( ! isset( $content_width ) )
	$content_width = 1140;


add_action( 'after_setup_theme', 'delphinus_theme_setup' );
if ( ! function_exists( 'delphinus_theme_setup' ) ):

function delphinus_theme_setup() {
    /**
     * Editor style.
     */
    ...
    * provide it for us.
    ...
    if (function_exists( 'add_image_size' ) ) {
        add_image_size( 'delphinus_grid', 570, 380, true);
        add_image_size( 'delphinus_masonry', 570);
        add_image_size( 'delphinus_square', 500, 500, true);
        add_image_size( 'delphinus_list', 700, 570, true);
        add_image_size( 'delphinus_classic', 1140, 600, true );
        add_image_size( 'delphinus_small', 150, 150, true );
        add_image_size( 'delphinus_widgets', 100, 75, true );
    }
    
    load_theme_textdomain( 'delphinus', DELPHINUS_THEME_DIR . '/languages' );
	...
	register_nav_menus(array(
        ...
        'footer_bottom'     => esc_html__( 'Footer Bottom Navigation Menu', 'delphinus' ),
        'footer'            => esc_html__( 'Footer Copyright Navigation Menu', 'delphinus' )
    ));
...
function delphinus_add_scripts() {

    wp_enqueue_script( 'html5shiv', DELPHINUS_THEME_JS.'html5shiv.min.js' );
    wp_script_add_data( 'html5shiv', 'conditional', 'lt IE 9' );

    wp_enqueue_script( 'respond', DELPHINUS_THEME_JS . 'respond.min.js' );
    wp_script_add_data( 'respond', 'conditional', 'lt IE 9' );

    wp_enqueue_style( 'bootstrap', DELPHINUS_THEME_LIBS . 'bootstrap/css/bootstrap.css', array());
    wp_enqueue_style( 'font-awesome', DELPHINUS_THEME_LIBS . 'font-awesome/css/font-awesome.min.css', array());
    wp_enqueue_style( 'delphinus-font', DELPHINUS_THEME_LIBS . 'delphinus/style.min.css', array());

    wp_enqueue_style( 'delphinus-plugins', DELPHINUS_THEME_CSS . 'plugins.css', array());


    if(delphinus_is_wc()){
        wp_enqueue_style( 'delphinus-woocommerce', DELPHINUS_THEME_CSS . 'woocommerce.css' );
    }

	// Load our main stylesheet.
    wp_enqueue_style( 'delphinus-main', DELPHINUS_THEME_CSS . 'style.css');
    wp_enqueue_style( 'delphinus-queries', DELPHINUS_THEME_CSS . 'queries.css');

	// Load the Internet Explorer specific stylesheet.
	wp_enqueue_style( 'delphinus-ie', DELPHINUS_THEME_CSS . 'ie.css');
	wp_style_add_data( 'delphinus-ie', 'conditional', 'lt IE 9' );
    ...
    if( $use_loader ){
        wp_enqueue_script( 'delphinus-pace', DELPHINUS_THEME_JS . 'pace.min.js', array( 'jquery' ), null );
    }

    wp_enqueue_script( 'bootstrap', DELPHINUS_THEME_LIBS . 'bootstrap/js/bootstrap.min.js', array( 'jquery' ), null, true );
    wp_enqueue_script( 'delphinus-plugins', DELPHINUS_THEME_JS . 'plugins.js', array( 'jquery' ), null, true );
    wp_enqueue_script( 'delphinus-main-script', DELPHINUS_THEME_JS . 'functions.js', array( 'jquery', 'mediaelement', 'wp-mediaelement' ), null, true );

    wp_localize_script( 'delphinus-main-script', 'ajax_frontend', array(
    ...
    if(delphinus_is_wc()){
        ...
        $shop_header = delphinus_option('shop_header_tool_bar', 1);
        if($shop_header == 2){
            if($shop_header_ajax = delphinus_option('shop_header_ajax', 1)){
                $ajax_filter = 1;
            }
        }
        ...
        wp_enqueue_script( 'delphinus-woocommerce', DELPHINUS_THEME_JS . 'woocommerce.js', array( 'jquery' ), null, true );
        wp_localize_script( 'delphinus-woocommerce', 'delphinus_woocommerce', $woo_attr);
    }

}
add_action( 'wp_enqueue_scripts', 'delphinus_add_scripts' );
 ...
 * @access      public
 */
function delphinus_setting_script() {

    $advanced_css = delphinus_option('advanced_editor_css');
    $css = $advanced_css;

    $styling_link = delphinus_option('styling_link');
    ...
    if(is_page() || is_singular() || $is_shop){
        ...
        $pageh_spacing = delphinus_meta('_delphinus_page_top_spacing', array(), $post_id);
        if($pageh_spacing != ''){
            $css .= '.content-area-inner{padding-top: '.$pageh_spacing.';}';
        }
        $pageh_spacing = delphinus_meta('_delphinus_page_bottom_spacing', array(), $post_id);
        if($pageh_spacing != ''){
            $css .= '.content-area-inner{padding-bottom:'.$pageh_spacing.';}';
        }

        $pageh_top = delphinus_meta('_delphinus_page_header_top', array(), $post_id);
        if($pageh_top != ''){
            $css .=  'div.page-header{padding-top: '.$pageh_top.';}';
        }

        $pageh_bottom = delphinus_meta('_delphinus_page_header_bottom', array(), $post_id);
        if($pageh_bottom != ''){
            $css .=  'div.page-header{padding-bottom: '.$pageh_bottom.';}';
        }

        $pageh_title_color = delphinus_meta('_delphinus_page_header_title_color', array(), $post_id);
        if($pageh_title_color != ''){
            $css .= 'div.page-header .page-header-title{color:'.$pageh_title_color.';}';
        }

        $pageh_subtitle_color = delphinus_meta('_delphinus_page_header_subtitle_color', array(), $post_id);
        if($pageh_subtitle_color != ''){
            $css .= 'div.page-header .page-header-subtitle{color:'.$pageh_subtitle_color.';}';
        }

        $pageh_breadcrumbs_color = delphinus_meta('_delphinus_page_header_breadcrumbs_color', array(), $post_id);
        if($pageh_breadcrumbs_color != ''){
            $css .= 'div.page-header .woocommerce-breadcrumb{color:'.$pageh_breadcrumbs_color.';}';
        }

        $css .= delphinus_render_custom_css('_delphinus_page_header_background', 'div.page-header', $post_id);

    }

    if($navigation_space = delphinus_option('navigation_space', 20)){
        $css .= '#nav #main-navigation > li + li{margin-left: '.$navigation_space.'px;}';
    }

    if($mega_border_color = delphinus_option('mega_border_color', '#ebebeb')){
        $mega_border_arr = array(
            ...
            '#nav #main-navigation > li .delphinus-megamenu-wrapper > ul > li .widget-title',
    ...
    if($toolbar_color = delphinus_option('header_toolbar_border_color', '#ebebeb')){
    ...
    if($toolbar_color_light = delphinus_option('header_toolbar_light_border_color', array( 'color' => '#f6f6f6', 'alpha' => '.2' ))){
        ...
        $css .= implode($toolbar_color_light_arr, ',').'{border-color: '.delphinus_hex2rgba($toolbar_color_light['color'], $toolbar_color_light['alpha']).';}';
    }

    if($navigation_height = delphinus_option('navigation_height', 102)){
    ...
    if($navigation_height_fixed = delphinus_option('navigation_height_fixed', 102)){
    ...
    $header_sticky_opacity = delphinus_option('header_sticky_opacity', 0.8);
    ...
    wp_add_inline_style( 'delphinus-main', $css );
}
add_action('wp_enqueue_scripts', 'delphinus_setting_script');


if ( ! function_exists( 'delphinus_excerpt_more' ) ) :
    /**
     ...
     */
    function delphinus_excerpt_more( $more ) {
        return ' &hellip; ';
    }
    add_filter( 'excerpt_more', 'delphinus_excerpt_more' );
endif;


if ( ! function_exists( 'delphinus_excerpt_length' ) ) :
    /**
    ...
    function delphinus_excerpt_length( ) {
        if(is_search()){
            $excerpt_length = delphinus_option('search_excerpt_length', 35);
        }else{
            $excerpt_length = delphinus_option('archive_excerpt_length', 40);
        }
        return $excerpt_length;
    }
    add_filter( 'excerpt_length', 'delphinus_excerpt_length');
endif;

if ( ! function_exists( 'delphinus_posts_per_page' ) ) :
    /**
     * Control the number of posts per page
     */
    function delphinus_posts_per_page( $query ) {
        if ( $query->is_main_query() && !is_admin()) {
            if(isset($_REQUEST['per_page'])){
                $posts_per_page = $_REQUEST['per_page'];
            }elseif(is_search()){
                $posts_per_page = delphinus_option('search_posts_per_page', 9);
            }elseif($query->is_category() || $query->is_home() || $query->is_tag() || $query->is_posts_page()){
                $posts_per_page = delphinus_option('archive_posts_per_page', 14);
            }
    ...
    add_action( 'pre_get_posts', 'delphinus_posts_per_page' );
endif;
/**
 *
 ...
 * @param $depth
 */
function delphinus_comments($comment, $args, $depth) {
...
if ( ! function_exists( 'delphinus_comment_nav' ) ) :
    /**
     * Display navigation to next/previous comments when applicable.
     *
     */
    function delphinus_comment_nav() {
...
if ( ! function_exists( 'delphinus_post_thumbnail_image' ) ) {
    /**
     ...
     *
     */
    function delphinus_post_thumbnail_image($size = 'post-thumbnail', $class_img = '', $link = true, $placeholder = false, $echo = true) {
        if ( is_attachment()) {
            return;
        }
        ...
        if(has_post_thumbnail() || $placeholder){ ?>
            ...
            <?php }elseif($placeholder){ ?>
                <?php
                    $image = apply_filters( 'delphinus_placeholder', $size );
                    printf(
...
if ( ! function_exists( 'delphinus_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time and author.
	 */
	function delphinus_posted_on() {
		...
		echo apply_filters( 'delphinus_single_post_posted_on_html', '<span class="posted-on">' . $posted_on . '</span><span class="byline"> ' . $byline . '</span>', $posted_on, $byline );

	}
}


if ( ! function_exists( 'delphinus_paging_nav' ) ) {
    /**
     * Display navigation to next/previous set of posts when applicable.
     */
    function delphinus_paging_nav( $type = 'normal' ) {
        ...
        <?php }elseif($type == 'loadmore'){ ?>
            <?php $more_link = get_next_posts_link( esc_html__( 'Load More', 'delphinus' ) ); ?>
            <?php if(!empty($more_link)){ ?>
                <nav class="navigation pagination-loadmore">
                    <?php echo get_next_posts_link( esc_html__( 'Load More', 'delphinus' ) ); ?>
                </nav>
            <?php } ?>
        <?php }else{
...
if ( ! function_exists( 'delphinus_entry_meta' ) ) {
	/**
	 * Display the post meta
	 * @since 1.0.0
	 */
	function delphinus_entry_meta( $arrays = array('categories', 'author') ) {
	    if ( 'post' == get_post_type() ) { ?>
            <div class="entry-post-meta">
			<?php
			foreach($arrays as $array ){
			    if($array == 'categories'){
                    delphinus_post_meta_categories();
			    }elseif($array == 'author'){
			        delphinus_post_meta_author();
			    }elseif($array == 'comments'){
			        delphinus_post_meta_comments();
			    }
			}
			?>
            </div>
...
if ( ! function_exists( 'delphinus_post_meta' ) ) {
	/**
	 * Display the post meta
	 * @since 1.0.0
	 */
	function delphinus_post_meta() {
		?>
		<div class="entry-meta">
			...
			delphinus_post_meta_categories();
			//delphinus_post_meta_author();
			delphinus_post_meta_date();
			delphinus_post_meta_comments();
			delphinus_entry_share_social();
...
if ( ! function_exists( 'delphinus_post_meta_comments' ) ) {
    function delphinus_post_meta_comments(){
...
if ( ! function_exists( 'delphinus_post_meta_categories' ) ) :
    /**
     * Prints HTML with meta information for categories.
     *
     */
    function delphinus_post_meta_categories( $separator = ', ') {
...
if ( ! function_exists( 'delphinus_post_meta_tags' ) ) :
    /**
     * Prints HTML with meta information for tags.
     *
     */
    function delphinus_post_meta_tags($separator = ', ', $before = '', $after = '') {
...
if ( ! function_exists( 'delphinus_post_meta_author' ) ) :
    /**
     * Prints HTML with meta information for author.
     *
     */
    function delphinus_post_meta_author() {
...
if ( ! function_exists( 'delphinus_post_meta_date' ) ) {
    /**
     * Prints HTML with date information for current post.
     *
     */
    function delphinus_post_meta_date() {
...
if ( ! function_exists( 'delphinus_entry_date' ) ) {
    /**
     * Prints HTML with date information for current post.
     *
     */
    function delphinus_entry_date() {
...
if ( ! function_exists( 'delphinus_entry_share_social' ) ) {
    function delphinus_entry_share_social() {
        $total = delphinus_entry_share_total();
        printf(
...
if ( ! function_exists( 'delphinus_entry_share_total' ) ) {
    function delphinus_entry_share_total() {
            ...
            if( !$cache ){
                $arr = array('facebook', 'twitter', 'google', 'linkedin', 'pinterest', 'stumbleupon');
                $total = 0;
                foreach( $arr as $social){
                    $total += delphinus_entry_share_count($social);
                }
...
if(!function_exists('delphinus_entry_share_count')){
    /**
     * Get count share for post
     * @param $service
     * @return int
     */
    function delphinus_entry_share_count( $service ) {
        ...
        if( !$cache ){
            ...
            $link = 'http://crestaproject.com/downloads/cresta-social-share-counter/';
            ...
            $hour = apply_filters('delphinus_caches_time_share', 4);
...
if ( ! function_exists( 'delphinus_entry_excerpt' ) ) :
	/**
	 * Displays the optional excerpt.
	 *
	 * Wraps the excerpt in a div element.
	 *
	 * Create your own delphinus_entry_excerpt() function to override in a child theme.
	 *
	 *
	 * @param string $class Optional. Class string of the div element. Defaults to 'entry-summary'.
	 */
	function delphinus_entry_excerpt( $class = 'entry-content' ) {
 ...
 * Share Box [share_box]
 * --------------------------------------------------------------------------- */
if( ! function_exists( 'delphinus_share_box' ) ){
    function delphinus_share_box($post_id = null, $style = "", $class = 'share-it', $count = false){
        global $post;
        ...
        $html = '';
        $share_arr = delphinus_option('social_share', array('facebook' => true, 'twitter' => true, 'pinterest' => true));
            ...
            foreach($share_arr as $key => $val){
                if($val){
                    ...
                    if($key == 'facebook'){
                        ...
                        if($count){
                            $html .= '<span class="count">'.delphinus_entry_share_count('facebook').'</span>';
                        }
                        $html .= '</a></li>';
                    }elseif($key == 'twitter'){
                        // Twitter
                        ...
                        if($count){
                            $html .= '<span class="count">'.delphinus_entry_share_count('twitter').'</span>';
                        }
                        $html .= '</a></li>';
                        ...
                        if($count){
                            $html .= '<span class="count">'.delphinus_entry_share_count('google').'</span>';
                        }
                        $html .= "</a></li>";
                        ...
                        if($count){
                            $html .= '<span class="count">'.delphinus_entry_share_count('pinterest').'</span>';
                        }
                        $html .= "</a></li>";
                    }elseif($key == 'linkedin'){
                        // linkedin
                        ...
                        if($count){
                            $html .= '<span class="count">'.delphinus_entry_share_count('linkedin').'</span>';
                        }
                        $html .= "</a></li>";
                    }elseif($key == 'tumblr'){
                        // Tumblr
...
if ( ! function_exists( 'delphinus_post_nav' ) ) {
	/**
	 * Display navigation to next/previous post when applicable.
	 */
	function delphinus_post_nav() {
		...
		$args = array(
			'next_text' => '<span class="meta-nav">'.esc_html__('Next Post', 'delphinus').'</span><span class="meta-title">%title</span>',
			'prev_text' => '<span class="meta-nav">'.esc_html__('Previous Post', 'delphinus').'</span><span class="meta-title">%title</span>',
        );
 ...
 * --------------------------------------------------------------------------- */
if ( ! function_exists( 'delphinus_related_article' ) ) :
    function delphinus_related_article($post_id = null, $type = 'categories'){
        global $post;
        if(!$post_id) $post_id = $post->ID;

        $posts_per_page = delphinus_option('blog_related_sidebar', 5);
        $excerpt_length = 15;
        ...
        $args = apply_filters('delphinus_related_article_args', $args);
