...
class KTMegaWalker extends Walker_Nav_Menu{
    ...
    public function start_lvl( &$output, $depth = 0, $args = array() ) {
        ...
        if($this->megamenu_enable == "enabled" && $depth == 0){
            $output .= "\n$indent<ul class=\"delphinus-megamenu-ul clearfix\">\n";
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $classes[] = 'delphinus-menu-item';


        if( $depth === 0 && $this->megamenu_enable) {
            $classes[] = 'delphinus-megamenu-item';
            $classes[] = 'delphinus-megamenu-item-'.$this->megamenu_width;
        }
        $endrow = get_post_meta( $item->ID, '_menu_item_megamenu_endrow', true );
        if($depth == 1 && $endrow){
            $classes[] = 'delphinus-megamenu-item-endrow';
        }
        ...
        $atts['class']   = 'delphinus-megamenu-link';
        ...
        if(($this->megamenu_enable && $depth == 1)){
            ...
            if($image){
                $file = delphinus_get_thumbnail_attachment($image, 'full');
                if($file['src']){
        ...
        if( $depth === 0 && $this->megamenu_enable == "enabled" ) {
            ...
            $item_output .= "\n$indent<div class=\"delphinus-megamenu-wrapper $position $layout megamenu-columns-$colums \">\n";
            ...
            if ( $depth == 0 && $item->object == 'category' ) {
                ...
                global $post;
                $menu_posts = get_posts( apply_filters('delphinus_megamenu_posts', array( 'posts_per_page' => 3, 'category' => $item->object_id )) );
        ...
        if($megamenu_widget && $depth ==1 ){
            ...
            $item_output .= '<div class="delphinus-megamenu-widget">'.ob_get_contents().'</div>';
            ob_end_clean();
        }
